import { adicionar_um_elemento_a_uma_lista, verifica_se_um_elemento_esta_em_uma_lista } from './funcoes_array.js'
...
function verificaSeCaractereEstaEmCaixaAlta(caractere) {
...
function verificaSeEhLetra(caractere = '') {
...
function verificaSeCaractereEstaEmCaixaBaixa(caractere) {
...
export function quebrar_texto(texto, caractere_de_separacao) {
    ...
    for (let indice_texto_atual = 0; indice_texto_atual < contar_tamanho_do_texto(texto); indice_texto_atual++) {
        if (texto[indice_texto_atual] == caractere_de_separacao[0]) {
            for (let indice_caractere_atual = 0; indice_caractere_atual < contar_tamanho_do_texto(caractere_de_separacao); indice_caractere_atual++) {
                if (texto[indice_texto_atual + indice_caractere_atual] == caractere_de_separacao[indice_caractere_atual]) {
                    numero_de_caracteres_procurados_que_estao_na_sequecia_esperada_para_formar_o_texto++

                }

            }
            if (numero_de_caracteres_procurados_que_estao_na_sequecia_esperada_para_formar_o_texto == contar_tamanho_do_texto(caractere_de_separacao)) {
                lista_das_partes_do_texto_separadas.push(parte_atual);
                //pular para o proximo idice apos o ultimo caratere_de_separacao
                //proximo idice= icremeto de (tamho do caratere_de_separacao - 1) + icremeto de 1( DADO AUTOMATICAMETE PELO LACO FOR AO TERMIAR A VOLTA ATUAL)
                indice_texto_atual += contar_tamanho_do_texto(caractere_de_separacao) - 1
                parte_atual = ''
        ...
        for (const caractere_atual of texto) {
            ...
            if (verificaSeEhLetra(caractere_atual)) {
                if (verificaSeCaractereEstaEmCaixaAlta(caractere_atual)) {
                    if (verifica_se_caractere_esta_no_texto(caracteres_permitidos, caractere_atual) == false || verifica_se_caractere_esta_no_texto(caracteres_permitidos, converterTextoMaiusculoEmMinusculo(caractere_atual)) == false) {
                    ...
                    if (verifica_se_caractere_esta_no_texto(caracteres_permitidos, caractere_atual) == false || verifica_se_caractere_esta_no_texto(caracteres_permitidos, caractere_atual) == false) {
                        usa_somente_os_caracteres = false
                        return usa_somente_os_caracteres
                    } else {
                        usa_somente_os_caracteres = true
