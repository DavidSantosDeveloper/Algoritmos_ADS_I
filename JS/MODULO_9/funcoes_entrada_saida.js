...
export function obter_numero_entre_intervalo(label = 'Digite um numero: ', inicio, fim) {
    let numero = obter_texto(label)

    while ((isNaN(Number(numero)) || numero === '') && numero >= inicio && numero <= fim) {
        mostrar_texto('Valor inválido!')
        numero = question(label)
    }
...
export function obter_decimal_ate_3_digitos(label) {
    let numero = obter_texto(label)
    if (numero.length <= 3) {
        while (isNaN(Number(numero)) || numero === '') {
            mostrar_texto('Valor inválido!')
        ...
        mostrar_texto('Valor inválido!')
        obter_decimal_ate_3_digitos(label)
    }
